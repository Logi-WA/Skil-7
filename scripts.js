...
function formatPrice(price) {
  return price.toLocaleString("is-IS", { style: "currency", currency: "ISK" });
}
...
function formatProduct(product, quantity = undefined) {
  if (quantity > 1) {
    return `${product.title} - ${quantity}x${formatPrice(product.price)}`;
  } else {
...
function cartInfo(cart) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    cartString += `${formatProduct(product, quantity)}\n`;
  }
  return `${cartString}Samtals: ${formatPrice(total)}`;
}
...
function showProducts() {
  ...
  for (let i = 0; i < size; i++) {
    ...
    productStrings.push(`#${product.id} ${formatProduct(product)}`);
  }

  console.info(productStrings.join("\n"));
}
...
function addProductToCart() {
  const productId = Number(prompt("Auðkenni vöru:"));
  ...
  const quantity = Number(prompt("Fjöldi (milli 1 - 99):"));
  ...
  if (cartLine) {
    cartLine.quantity += parseInt(quantity);
  } else {
    cart.lines.push({
      product: product,
      quantity: quantity,
...
function checkout() {
  ...
  if (!name || !address) {
    console.error("Nafn og heimilisfang verða að vera skilgreind.");
    return;
  }

  const checkoutMsg = [
    "Pöntun móttekin " + name + ".",
    "Vörur verða sendar á " + address + "." + "\n",
    cartInfo(cart)
  ].join("\n");

  console.info(checkoutMsg);
}
